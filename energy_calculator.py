...
def convert_to_list_representation(circuit_of_gates):
    """
    Convert a quantum circuit composed of Gate objects into a simple list-based notation.
    This version maps CNOT gates to 'cx'.
    """
    raw_gate_list = []
    for gate in circuit_of_gates:
        ...
        if qubit_indices is None or not isinstance(qubit_indices, list):
            print(f"Warning: Invalid 'act_on' attribute for Gate '{gate.name}', skipped.")
            continue
...
class VqeTrainerNew:
    ...
    def train_circuit(self, work_item):
        """
        A single run of training a single circuit. This is the smallest computational unit.
        """
    ...
    def train_and_save_batch(self, circuit_batch, batch_index, batch_results_dir):
        print(f"--- Start efficient parallel training batches {batch_index} ( {len(circuit_batch)} circuits) ---")
        ...
        print("All parallel tasks have been completed, starting to process the results...")
        ...
        for i, single_circuit_results in enumerate(grouped_results):
            if not single_circuit_results:
                print(f"Warning: No training results were received for circuit {i} in batch {batch_index}, which may indicate a problem.")
                continue
    ...
    def run_batched_training(self, circuits, task_name, run_id, exp_dir, batch_size=200):
        print(f"\nUsing seed {run_id} to fix global randomness...")
        np.random.seed(run_id)
        tf.random.set_seed(run_id)
        random.seed(run_id)
        print("Numpy, TensorFlow, and Random modules' seeds have been fixed.")
        ...
        print(f"Batch results save directory: {batch_results_dir}")
        ...
        print(f"Total number of quantum circuits to be trained: {total_circuits}")
        ...
        for i in range(0, total_circuits, batch_size):
            ...
            if os.path.exists(batch_result_file):
                print(f"Batch {batch_index} result file already exists, skipping training.")
                continue
        ...
        print(
            f"\nAll batches of training have been completed. Total training time: {int(total_duration // 3600)} hours {int((total_duration % 3600) // 60)} minutes")
        utils.save_pkl(total_duration, os.path.join(exp_dir, 'duration_train.pkl'))

    def merge_results(self, task_name, run_id, exp_dir):
        """
        Merge all batch training result files to generate the final summary file.
        """
        exp_dir = exp_dir
        batch_results_dir = os.path.join(exp_dir, "batch_results")

        if not os.path.exists(batch_results_dir):
            print(f"Error: Could not find the batch results directory {batch_results_dir}. Please check if the training has been completed.")
            return

        print("\n--- Start merging all batch results ---")
        ...
        try:
            ...
            print(f"Error: Failed to read or sort batch result files. Error: {e}")
            return

        if not result_files:
            print("Error: No batch result files found to merge. Please check if the training has been completed.")
            return
        ...
        for filename in result_files:
            ...
            try:
                ...
                print(f"Warning: Failed to load or process file {filename}. Error: {e}")
        ...
        print(f"\nSuccessfully merged {len(result_files)} batch result files. Total number of circuits: {len(energy_final)}")
        print(f"Final summary files have been saved to: {exp_dir}")

def calculate_true_energies(
        seed,
        qubit,
        task_name,
        circuits,
    ...
    # 噪声信息
    two_qubit_depolarizing_p = 0.01 # Magnitude of two-qubit depolarizing noise
    single_qubit_depolarizing_p = 0.001 # Magnitude of single-qubit depolarizing noise
    bit_flip_p = 0.01 # Magnitude of bit-flip noise
    if noise:
    ...
    if hamiltonian is None:
        raise ValueError("No Hamiltonian has been set.")
